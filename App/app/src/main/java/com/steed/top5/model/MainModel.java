...
//                                        }
//                                    });
                                    firebaseDatabase.child("content").child("posts").child(postID).child("likes").addListenerForSingleValueEvent(new ValueEventListener() {
                                        @Override
                                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                                            if(snapshot.exists()){
                                                final long likeCount = Long.valueOf(snapshot.getValue().toString());
                                                currPost.likes = likeCount;
                                            }
                                    ...
                                    firebaseDatabase.child("users").child("regularUsers").child(userSingleton.currentUser.uid).child("saved").child(currPost.id).addListenerForSingleValueEvent(new ValueEventListener() {
                                        ...
                                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                                            ...
                                            firebaseDatabase.child("content").child("posts").child(post.getKey()).child("likedUsers").child(userSingleton.currentUser.uid).addListenerForSingleValueEvent(new ValueEventListener() {
                                                @Override
                                                public void onDataChange(@NonNull DataSnapshot snapshot) {
            ...
            public void onComplete(@Nullable DatabaseError error, boolean committed, @Nullable DataSnapshot currentData) {
                if (committed) {
                    if (post.isLiked) {
                        ...
                        firebaseDatabase.child("users").child("regularUsers").child(userSingleton.currentUser.uid).child("tags").setValue(tagsSingleton.tags).addOnCompleteListener(new OnCompleteListener<Void>() {
                            ...
                            public void onComplete(@NonNull Task<Void> task) {
                                firebaseDatabase.child("users").child("regularUsers").child(userSingleton.currentUser.uid).child("recentlyViewed").setValue(tagsSingleton.recentlyViewed).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    @Override
                                    public void onComplete(@NonNull Task<Void> task) {
                                        firebaseDatabase.child("content").child("posts").child(post.id).child("likedUsers").child(userSingleton.currentUser.uid).setValue("userID").addOnCompleteListener(new OnCompleteListener<Void>() {
                                            @Override
                                            public void onComplete(@NonNull Task<Void> task) {
                        ...
                        firebaseDatabase.child("content").child("posts").child(post.id).child("likedUsers").child(userSingleton.currentUser.uid).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                            @Override
                            public void onComplete(@NonNull Task<Void> task) {
                                postLikedOrUnlikedLiveData.setValue(new LikeResponse(task.isSuccessful(), post));
                            }
    ...
    public MutableLiveData<String> getAllPostLikerString(String postID, boolean isLiked, boolean isLangEn) {
        ...
        firebaseDatabase.child("content").child("posts").child(postID).child("likedUsers").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
...
//                    final String[] likeStr = {isLiked ? "You" : ""};
                    ...
                    if(totalLikes == 1){
                        if(isLiked){
                            likeStr[0] += "liked this.";
                        }else {
                            for (DataSnapshot likerz : snapshot.getChildren()){
                                firebaseDatabase.child("users").child("regularUsers").child(likerz.getKey()).child("name").addListenerForSingleValueEvent(new ValueEventListener() {
                                    @Override
                                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                                        likeStr[0] = ""+snapshot.getValue()+" liked this.";
                                    }
                    ...
                    else if(totalLikes == 2 && isLiked == true){
//                        likeStr[0] = "You and 1 other liked this.";
                        for (DataSnapshot likerz : snapshot.getChildren()){
                            if(!likerz.getKey().equals(userSingleton.currentUser.uid)){
                                firebaseDatabase.child("users").child("regularUsers").child(likerz.getKey()).child("name").addListenerForSingleValueEvent(new ValueEventListener() {
                                    @Override
                                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                                        likeStr[0] = "You and "+snapshot.getValue()+" liked this.";
                                    }
...
//                        String[] lastLikedNames = new String[totalLikes];
                            ...
                            if (!liker.getKey().equals(userSingleton.currentUser.uid)) {
                                ...
                                firebaseDatabase.child("users").child("regularUsers").child(liker.getKey()).child("name").addListenerForSingleValueEvent(new ValueEventListener() {
                                    ...
                                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                                        ...
                                        Log.e("571",snapshot.getValue().toString());
                                        if (namedUsers.size() == limit || finalTotalLikes - namedUsers.size() <= 0) {
                                            long remainingLikes = finalTotalLikes - (namedUsers.size()-1);
                                            for (int j = 0; j < namedUsers.size()-1; j++) {
                                                if (j != 0 && j == namedUsers.size() - 1 && remainingLikes == 1) {
                                                    likeStr[0] += " and ";
                                                } else {
                                                    if (!(remainingLikes > 0 && j == namedUsers.size()) && likeStr[0].length() > 0) {
                                                        likeStr[0] += ", ";
                                                    }
                                                ...
                                                if(finalTotalLikes == 1){likeStr[0] += " liked this."; }
                                                else if((finalTotalLikes == 2)){likeStr[0] += " and " + namedUsers.get(j+1) + " liked this.";}
//                                                else if((finalTotalLikes == 3)){likeStr[0] += ", " + namedUsers.get(j+1) +" and " + namedUsers.get(j+2) + " liked this.";}
//                                                else if((finalTotalLikes > 2)){likeStr[0] +=  ", "+remainingLikes+"";}
                                                else if((finalTotalLikes > 2)){
                                                    if(isLiked == true){likeStr[0] += " and "+ remainingLikes +" others liked this.";}else{likeStr[0] += ", " + namedUsers.get(j+1)  + " and "+ remainingLikes +" others liked this.";}
//                                                    likeStr[0] += ", " + namedUsers.get(j+1)  + " and "+ remainingLikes +" others liked this.";
                                                break;
                                                }
//                                                else{likeStr[0] += " ";}
//                                               System.out.println(likeStr[0]);
//                                                lastLikedNames[j] = namedUsers.get(j);

                                            }

//                                            if ((finalTotalLikes > 1) && (remainingLikes > 0)) {
//                                                likeStr[0] += " and " + remainingLikes + " other like this";
//                                            }else if ((finalTotalLikes == 1) && (remainingLikes > 0)) {
//                                                likeStr[0] +=  remainingLikes + " like this";
//                                            } else {
//                                                likeStr[0] += " like this";
//                                            }
//                                            if ((finalTotalLikes > 1) && (remainingLikes > 0)) {
...
//                                            if(finalTotalLikes == 1){likeStr[0] += " liked this."; }
//                                            else if((finalTotalLikes == 2)){likeStr[0] += " and " + likeStr[0] + " liked this.";}
//                                            else if((finalTotalLikes == 3)){likeStr[0] += ", " + likeStr[0] +" and " + likeStr[2] + " liked this.";}
//                                            else if((finalTotalLikes > 3)){likeStr[0] += ", " + likeStr[0] +", " + likeStr[0] + " and "+ remainingLikes + " others liked this.";}
//                                            else{likeStr[0] += " ";}
                        ...
                        if (namedUsers.isEmpty()) {
                            allPostLikerLiveData.setValue(isLangEn ? "You liked this" : "اینو دوست داری");
                        }
                    }
                                    ...
                                    if (postSnapshot.exists()) {
                                        ...
                                        firebaseDatabase.child("users").child("regularUsers").child(userSingleton.currentUser.uid).child("saved").child(currPost.id).addListenerForSingleValueEvent(new ValueEventListener() {
                                            ...
                                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                ...
                                                firebaseDatabase.child("content").child("posts").child(postID).child("likedUsers").child(userSingleton.currentUser.uid).addListenerForSingleValueEvent(new ValueEventListener() {
                                                    @Override
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    firebaseDatabase.child("users").child("regularUsers").child(userSingleton.currentUser.uid).child("saved").child(currPost.id).addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            ...
                            firebaseDatabase.child("content").child("posts").child(postSnapshot.getKey()).child("likedUsers").child(userSingleton.currentUser.uid).addListenerForSingleValueEvent(new ValueEventListener() {
                                @Override
                                public void onDataChange(@NonNull DataSnapshot likesSnapshot) {
                                    currPost.isLiked = likesSnapshot.exists();
