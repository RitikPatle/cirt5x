...
def post(request, post_title_id):
    ...
    if request.method == "POST":
        if request.POST['type'] == "like":
            ...
            try:
                ...
                lastLikedNames = db.child("content/posts/" + postID + "/likedUsers/").get().val()
                # print(lastLikedNames)
                namesListX = []
                if lastLikedNames != None:
                    for val in lastLikedNames:
                        # print(val)
                        namesListX.append(db.child("users/regularUsers/" + val + "/name").get().val())
                # print("namesListX")
                #TODO:Check if user has liked post already
                # print("Hellou two")
                hasLiked = False if not "user" in request.session else db.child("content/posts/" + postID + "/likedUsers/" + request.session['uid']).get().val() != None
                ...
                if request.POST["isLike"] == "false" and hasLiked:
                    ...
                    if currLikeCount >= 0:
                        ...
                        namesListY = []
                        lastLikedNamez = db.child("content/posts/" + postID + "/likedUsers").get().val()
                        if lastLikedNamez != None:
                            for val in lastLikedNamez:
                                namesListY.append(db.child("users/regularUsers/" + val + "/name").get().val())
    ...
    if post != None:
        ...
        lastLikedNamec = db.child("content/posts/" + postID + "/likedUsers/").get().val()
            ...
            for val in lastLikedNamec:
                try:
                    if val == request.session['uid']:
                        ...
                        namesListK.append(db.child("users/regularUsers/" + val + "/name").get().val())
                except:
                    namesListK.append(db.child("users/regularUsers/" + val + "/name").get().val())
        ...
        # likes = db.child("likes/" + postID).get().val()
        likes = db.child("content/posts/" + postID + "/likedUsers/").get().val()
