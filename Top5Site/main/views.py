...
def home(request):
        ...
        if request.POST['type'] == "load":
                ...
                if(len(categories) > 0):
                    ...
                    if isLoggedIn:
                        ...
                        if len(loggedInUserCategoricalPosts) == 0:
                            ...
                            for categoryID, categoryValue in userPrefCategories.items():
                                if categoryID in categoryIDs:
                                    ...
                                    try:
                                        ...
                                        # print(len(categoricalPosts))

                                        # totalCategoricalPostsX += len(categoricalPosts)
                                        # print(totalCategoricalPostsX)
                            ...
                            # print(totalCategoricalPostsX)
                            # totalCategoricalPostsX = len(loggedInUserCategoricalPosts)
                            # print(totalCategoricalPostsX)

                        validPosts = []
                        # print(loggedInUserCategoricalPosts.length())
                        ...
                        for post in selectedPosts:
                            ...
                            isLiked = db.child(
                                "content/posts/"+post['id']+"/likedUsers/"+request.session['uid']).get().val() != None
                            # print(isLiked)
                            post["isLiked"] = 1 if isLiked else 0

                            lastLikedNames = db.child(
                                "content/posts/" + postID + "/likedUsers/").get().val()
                            namesListX = []
                            if lastLikedNames != None:
                                for val in lastLikedNames:
                                    namesListX.append(
                                        db.child("users/regularUsers/" + val + "/name").get().val())

                            currLikeCount = getLikeCount(post['id'])
                            post["likesCount"] = currLikeCount
                            if currLikeCount == 0:
                                ...
                                if currLikeCount == 2:
                                    likeStr = f"You and {namesListX[0]} liked this"
                                if currLikeCount == 3:
                                    likeStr = f"You, {namesListX[0]} and {namesListX[1]} liked this"
                                if currLikeCount > 3:
                                    likeStr = f"You, {namesListX[0]}, {namesListX[1]} and {currLikeCount - 3} others liked this"
                            else:
                                likeStr = ""
                            ...
                            tsdb = lastLikedNames = db.child(
                                "content/posts/" + postID + "/timestamp/").get().val()

                            post["dateTime"] = dateTimeFromTimeStamp(tsdb)

                            post["outerText"] = cleanhtml(post['text'])
                        ...
                        totalCategoricalPostsX = len(postIDs)
                    ...
                    return JsonResponse({"result": "success", "posts": allPosts, "totalPostsCount" : totalCategoricalPostsX,})
                else:
                    # print("=== JSON response 2")
                    return JsonResponse({"result": "success", "posts": [],"totalPostsCount" : 0})
            except Exception as e:
                exc_type, exc_obj, exc_tb = sys.exc_info()
            ...
            try:
                ...
                if request.POST["isLike"] == "true":
                    ...
                    db.child(
                        "content/posts/" + request.POST["postID"] + "/likedUsers/" + request.session['uid']).set("userID")
                else:
                    # db.child(
                    #     "likes/" + request.POST["postID"] + "/" + request.session['uid']).remove()
                    db.child(
                        "content/posts/" + request.POST["postID"] + "/likedUsers/" + request.session['uid']).remove()

                currLikeCount = getLikeCount(request.POST["postID"])
    ...
    lastThreePosts = db.child(
        "content/posts").order_by_key().limit_to_last(15).get().val()
    ...
    if lastThreePosts != None:
        ...
        for postID in lastThreePosts:
            ...
            post['cardTitle'] = db.child(
                "content/posts/"+postID+"/name/").get().val()
            post['likeCnt'] = db.child(
                "content/posts/"+postID+"/likes/").get().val()
            post['commentCnt'] = db.child(
                "content/posts/"+postID+"/comments/").get().val()
            post['categoryText'] = db.child(
                "content/posts/"+postID+"/category/").get().val()
    ...
    return render(request, "site/pages/home_infinite.html",
                  {
                      "isLoggedIn": "user" in request.session,
                      "lang": request.session['lang'] if "lang" in request.session else "en",
                      "staticTextMap": engAndPersianStaticText,
                      "userID": request.session['uid'] if "user" in request.session else "",
                      "categories": categories,
                      # "posts":posts,
                      "recentPosts": recentPosts,
                      "recentPostsCount": len(recentPosts),
                      "serverTime": int(math.floor(time.time() * 1000))
                  })
                    ...
                    if len(postIDs) > 0:
                        ...
                        for postID in posts:
                            ...
                            isLiked = False if not "user" in request.session else db.child(
                                "content/posts/" + postID + "/likedUsers/" + request.session['uid']).get().val() != None
                            post["isLiked"] = 1 if isLiked else 0
...
def post(request, post_title_id):
    ...
    # print(f"abc = {request.session['uid']}")
    ...
    if request.method == "POST":
        if request.POST['type'] == "like":
            ...
            try:
                ...
                # currLikeCount = db.child(
                #     "content/posts/" + postID + "/likes").get().val()
                currLikeCount = getLikeCount(postID)
                # print(f"abk = {currLikeCount}")
                # currLikeCount = 0 if currLikeCount == None else currLikeCount

                lastLikedNames = db.child(
                    "content/posts/" + postID + "/likedUsers/").get().val()
                # print(lastLikedNames)
                namesListX = []
                if lastLikedNames != None:
                    for val in lastLikedNames:
                        # print(val)
                ...
                if request.POST["isLike"] == "true" and not hasLiked:
                    ...
                    db.child(
                        "content/posts/" + postID + "/likedUsers/" + request.session['uid']).set("userID")

                if request.POST["isLike"] == "false" and hasLiked:
                    # print(f"came in {currLikeCount}")
                    if currLikeCount >= 0:
                        ...
                        db.child(
                            "content/posts/" + postID + "/likedUsers/" + request.session['uid']).remove()
                        namesListY = []
                        lastLikedNamez = db.child(
                            "content/posts/" + postID + "/likedUsers").get().val()
                        if lastLikedNamez != None:
                            for val in lastLikedNamez:
                                namesListY.append(
                                    db.child("users/regularUsers/" + val + "/name").get().val())
                        # likeStr = f"{namesListX} liked this."
                        if currLikeCount == 0:
                            # print("Hel else 0")
                            likeStr = "Be the first to like this."
                        if currLikeCount == 1:
                            # print("Hel else 1")
                            likeStr = f"{namesListY[0]} liked this"
                        if currLikeCount == 2:
                            # print("Hel else 2")
                            likeStr = f"{namesListY[0]} and {namesListY[1]} liked this"
                        if currLikeCount == 3:
                            likeStr = f"{namesListY[0]}, {namesListY[1]}, {namesListY[2]} liked this"
                        if currLikeCount > 3:
                            likeStr = f"{namesListY[0]}, {namesListY[1]}, {namesListY[2]} and {currLikeCount - 3} others liked this"
    ...
    if post != None:
        ...
        namesListK = []
        isPostLiked = False
        # lastLikedNamec = db.child("likes/" + postID).get().val()
        lastLikedNamec = db.child(
            "content/posts/" + postID + "/likedUsers/").get().val()
        # print(lastLikedNamec)

        if lastLikedNamec != None:
            for val in lastLikedNamec:
                try:
                    if val == request.session['uid']:
                        isPostLiked = True
                    else:
                        namesListK.append(
                            db.child("users/regularUsers/" + val + "/name").get().val())
        ...
        if isPostLiked == True:
            if currLikeCount == 0:
                ...
                if currLikeCount == 2:
                    likeStr = f"You and {namesListK[0]} liked this"
                if currLikeCount == 3:
                    likeStr = f"You, {namesListK[0]} and {namesListK[1]} liked this"
                if currLikeCount > 3:
                    likeStr = f"You, {namesListK[0]}, {namesListK[1]} and {currLikeCount - 3} others liked this"
            else:
                likeStr = ""
        else:
            if currLikeCount == 0:
                ...
                likeStr = f"{namesListK[0]} liked this"
            elif currLikeCount > 0:
                if currLikeCount == 2:
                    likeStr = f"{namesListK[0]} and {namesListK[1]} liked this"
                if currLikeCount == 3:
                    likeStr = f"{namesListK[0]}, {namesListK[1]}, {namesListK[2]} liked this"
                if currLikeCount > 3:
                    likeStr = f"{namesListK[0]}, {namesListK[1]}, {namesListK[2]}  and {currLikeCount - 3}  others liked this"
            else:
                likeStr = ""

        # print(likeStr,currLikeCount)

        # likeStr = "You liked thiszz" if isLiked else ""
        # likes = db.child("likes/" + postID).get().val()
        likes = db.child("content/posts/" + postID +
                         "/likedUsers/").get().val()
        # post["likesCount"] = getLikeCount(postID)
        # if likes != None:
        #     likes = list(dict(likes).keys())
        ...
        if commentItems != None:
            for key, val in commentItems.items():
                ...
                val["profilePhoto"] = db.child(
                    "users/regularUsers/" + val["userID"] + "/profilePhoto").get().val()
        ...
        if request.POST['type'] == "load":
            try:
                ...
                if len(categoriesItems) > 0:
                    ...
                    postIDs = {}
                    for word in words:
                        wordPostIDs = db.child("words").order_by_key().start_at(
                            word).end_at(word + "\uf8ff").get().val()
                        if wordPostIDs != None:
...
# def setFirstLike(postIDe,uIDe):
#     db.child("content/posts/"+ postIDe + "/likedUsers").set({uIDe : "userID"})


def getLikeCount(postIDe):
    # count = db.child("content/posts/" + postIDe +"/likes").get().val()
    count = db.child("content/posts/" + postIDe + "/likedUsers").get().val()
    # count = len(count)
    if count == None:
        count = 0
    else:
    ...
    return count
    # if count == None:
    #     db.child("content/posts/"+ postIDe + "/likedUsers").set({"IVxv7kxDnPRR9RLcpGUvpmL2Eaq1" : "userID"})
    #     exit()
...
def dateTimeFromTimeStamp(tsdb):
    datatsdb = int(tsdb) / 1000
    readable = datetime.datetime.fromtimestamp(datatsdb)
    date_time = readable.strftime("%H:%M:%S | %m/%d/%Y")
    return str(date_time)

# def addLikeCount(postIDe,uIDe):
#     prevCount = db.child("content/posts/" + postIDe +"/likes").get().val()
#     db.child("content/posts/" + postIDe +"/likes").set(prevCount + 1)
#     db.child("likes/" + postIDe + "/" + uIDe).set("userID")
#     count = getLikeCount(postIDe)
#     return count

# def subLikeCount(postIDe,uIDe):
#     prevCount = db.child("content/posts/" + postIDe +"/likes").get().val()
#     db.child("content/posts/" + postIDe +"/likes").set(prevCount - 1)
#     db.child("likes/" + postIDe + "/" + uIDe).remove()
#     count = getLikeCount(postIDe)
#     return count
